...
int main(void)
{
	...
	int termA = 0;
	int termB = 0;
	bool termComp = false; // Determines if the current Term is complete
	...
	while(1) {
	   ...
	   for (i = 0; i < 23; i++) {
	      if (inRange(buttons[i][0], buttons[i][2], touch.px/10) && inRange(buttons[i][1], buttons[i][3], touch.py/10)) {
	         //iprintf("\x1b[18;26H%d ", buttons[i][4]);
	      	 if (buttons[i][4] < 0) {
		    termComp = true;
			...
			termCurrent = false;
			  ...
			  case -10: // equal
			     ...
			     if (result != NULL) {
			        iprintf("\x1b[17;26H%d", result);
			     } else {
                                iprintf("\x1b[17;17HERROR");
			     }
                             //termA = result;
			     //termB = 0;
			     //operation = 0;
